...
std::vector<uint8_t> Secret::generateRandomBytes(size_t size) {
    ...
    std::generate(randomBytes.begin(), randomBytes.end(), [&]() { return distribution(generator); });
...
std::string Secret::getLatin1() const {
    return latin1Encode(bytes);
}

std::string Secret::getUTF8() const {
    return utf8Encode(bytes);
}

std::string Secret::getBase32() const {
    return base32Encode(bytes);
}

std::string Secret::getHex() const {
    return hexEncode(bytes);
}

// Getter for the secret key buffer.
const std::vector<uint8_t>& Secret::getBytes() const {
    return bytes;
}
